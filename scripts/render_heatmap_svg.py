...
def boustrophedon(nc, nr):
    path = []
    for ci in range(nc):
        rng = range(nr) if ci % 2 == 0 else range(nr-1,-1,-1)
        for ri in rng: path.append((ci, ri))
    for ci in range(nc-1,-1,-1): path.append((ci, 0))
    return path
...
def render(data):
    ...
    stats_h, canvas_h = 88, TITLEBAR_H + TOP_LABEL_H + art_h + 88 + PAD
    grid_top = TITLEBAR_H + TOP_LABEL_H
    grid_left = PAD + LEFT_LABEL_W
    snake_path = boustrophedon(n_cols, n_rows)
    ...
    css = "@keyframes cell{0%{opacity:0;transform:translateY(-6px)}100%{opacity:1;transform:translateY(0)}}.c{opacity:0;animation:cell .42s cubic-bezier(.2,.8,.2,1) both}"
    parts = [
